resource "tls_private_key" "baston_keypair" {
  algorithm = "RSA"
  rsa_bits  = 4096
}
...
resource "aws_key_pair" "baston_host_keypair" {
  ...
  public_key = tls_private_key.baston_keypair.public_key_openssh
}

resource "local_file" "P1_3tier_archi_keypair_private" {
  filename        = "P1_3tier_archi_keypair_private"
  file_permission = "0400"
  content         = tls_private_key.baston_keypair.private_key_pem
}